...
public final class Common {
  ...
  public static String colorize(final String textToTranslate) {
    ...
    for (int i = 0; i < mess.length;) { // i increment is handled case by case for speed
      ...
      if (doubleTag) { // DoubleTag module
        ...
        if (max <= mess.length) {
          ...
          for (int n = i; n < max; n++) {
            tmp = mess[n];
            // The order of the checks below is meant to improve performances (i.e. capital
            // letters check is at the end)
            if (!((tmp >= '0' && tmp <= '9') || (tmp >= 'a' && tmp <= 'f') || (tmp >= 'A' && tmp <= 'F'))) {
              // It wasn't a hex color, appending found chars to the StringBuilder and
    ...
    if (color) {
      ...
      // color and hashtag cannot be true at the same time
    ...
    if (hashtag) {
      ...
      // color, hashtag, and doubleTag cannot be true at the same time
      // Append "&##" if "&##" were the last characters of the string
    }
