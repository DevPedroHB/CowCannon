...
public final class Scheduler {
  ...
  public static void run(Runnable runnable) {
    if (isFolia) {
      Bukkit.getGlobalRegionScheduler().execute(CowCannon.getInstance(), runnable);
    } else {
  ...
  public static Task runLater(Runnable runnable, long delayTicks) {
    if (isFolia) {
      return new Task(
          Bukkit.getGlobalRegionScheduler().runDelayed(CowCannon.getInstance(), t -> runnable.run(), delayTicks));
    } else {
  ...
  public static Task runTimer(Runnable runnable, long delayTicks, long periodTicks) {
    if (isFolia) {
      return new Task(Bukkit.getGlobalRegionScheduler().runAtFixedRate(CowCannon.getInstance(), t -> runnable.run(),
