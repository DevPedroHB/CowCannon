...
public class Conversation {
  ...
  public void start() {
    this.conversation
        .append("The following is a conversation with an AI who represents a {role} NPC character in Minecraft. " +
  ...
  public boolean onTalk(String message) {
    if (this.npc.getLocation().distance(this.player.getLocation()) > 10) {
      Common.tell(this.player, "&che " + this.role + " stopped talking to you because you moved too far away.");
    ...
    if (this.player.hasMetadata("NPCTrade")) {
      if (message.equalsIgnoreCase("agree")) {
        ...
        if (!this.hasFreeSpace(npcItem, npcAmount)) {
          Common.tell(this.player,
              "&cYou don't have enough space in your inventory to buy " + npcAmount + " " + npcItem + ".");
        ...
        for (int i = 0; i < this.player.getInventory().getSize(); i++) {
          if (this.player.getInventory().getItem(i) != null
              && this.player.getInventory().getItem(i).getType() == playerItem) {
        ...
        Common.tell(this.player, "&8[&6!&8] &7You agreed to the trade. The " + this.role + " will now give you "
            + npcAmount + " " + npcItem + " and you will give him " + playerAmount + " " + playerItem + ".");
    ...
    new BukkitRunnable() {
      ...
      public void run() {
        ...
        if (type.equals("trade")) {
          ...
          Common.tell(player, "&8[&6!&8] &7Offer: To trade " + playerAmount + " " + playerItemFormatted + " for "
              + npcAmount + " " + npcItemFormatted + ".");
